document.addEventListener('DOMContentLoaded', () => {
    ...
    async function updateNowPlayingInfo() {
        ...
        try {
            ...
            const stationData = stations.find(data => data.station.id === currentStationId);
            if (!stationData) return;

            const nowPlaying = stationData.now_playing.song;
    ...
    document.getElementById('share-btn').addEventListener('click', (e) => {
        ...
        if (navigator.share) {
            ...
            navigator.clipboard.writeText(`${shareText} at ${url}`)
    ...
    fetchNowPlaying();
    setInterval(fetchNowPlaying, 30000); // every 30s
    setInterval(updateNowPlayingInfo, 10000); // every 10s
});
